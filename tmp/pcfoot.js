...
let _getHtml = _data => {
let html='';
html+= (function(link , jsmods){let str = '';let version = _data.version;let base = _data.staticHost + _data.pcPath + '/js/';if (link.length) {link.unshift('jquery');link.map(src => {str += '<script src="' + base + src + '.js?' + version + '"></script>';});return str;}if (jsmods.length) {jsmods.unshift('jquery');jsmods = '~' + jsmods.join('+');str += '<script  src="' + base + jsmods + '?' + version + '"></script>';}return str;})(_data._JSLinks ,_data._JSmods) 
html+=`<script>`
